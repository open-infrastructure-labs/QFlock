...
package org.apache.spark.sql.catalyst.optimizer.ext
 ...
 */
case class CostBasedJoinReorderExt(spark: SparkSession)
...
//      plan
//    } else {
      logInfo("CostBasedJoinReorderExt Starting...")
      val result = plan.transformDownWithPruning(_.containsPattern(INNER_LIKE_JOIN)) {
...
object JoinReorderDP extends PredicateHelper with Logging {
  ...
  def search(
    ...
    val foundPlans = mutable.Buffer[JoinPlanMap]({
      ...
      itemIndex.foreach {
        case (item, id) =>
          joinPlanMap.put(Set(id), JoinPlan(Set(id), item, ExpressionSet(), Cost(0, 0, 0)))
      }
      joinPlanMap
    })
    ...
    logDebug(s"Qflock Join reordering finished. Duration: $durationInMs ms, number of items: " +
  ...
  case class JoinPlan(
    ...
    def rootCost(conf: SQLConf): Cost = {
      if (itemIds.size > 1) {
        ...
        Cost(rootStats.rowCount.get, rootStats.sizeInBytes, 1)
      } else {
        // If the plan is a leaf item, it has zero cost.
        if ((plan.toString().contains("tpcds.store") ||
            plan.toString().contains("tpcds.item")) &&
          !plan.toString().contains("tpcds.store_sales")) {
          Cost(0, 0, 1000)
        } else {
          Cost(0, 0, 1)
        }
    ...
    def betterThan(other: JoinPlan, conf: SQLConf): Boolean = {
      if (other.planCost.card == 0 || other.planCost.size == 0) {
        ...
        if (other.planCost.distance > 1 || this.planCost.distance > 1) {
          (this.planCost.distance < other.planCost.distance)
        } else {
          val relativeRows = BigDecimal(this.planCost.card) / BigDecimal(other.planCost.card)
          val relativeSize = BigDecimal(this.planCost.size * this.planCost.distance) /
            BigDecimal(other.planCost.size * other.planCost.distance)
 ...
 */
case class Cost(card: BigInt, size: BigInt, distance: BigInt) {
  def +(other: Cost): Cost = Cost(this.card + other.card, this.size + other.size,
                                  this.distance.max(other.distance))
}
