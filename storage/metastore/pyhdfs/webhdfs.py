...
class WebHdfsFile(object):
    def __init__(self, name, user=None, buffer_size=(128 << 10), orig=None, storage_ip=None):
        self.name = name
        self.user = user
        ...
        if orig is None:
            ...
            if storage_ip is not None:
                location = location.replace('qflock-storage', storage_ip, 1)
            self.data_url = urllib.parse.urlparse(location)
            conn.close()
        else:
