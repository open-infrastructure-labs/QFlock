...
if __name__ == '__main__':
    ...
    for i in range(0, len(tables), 1):
        if 'hdfs://qflock-storage-dc2:' in tables[i].sd.location:
            tables[i].sd.location = tables[i].sd.location.replace('hdfs://qflock-storage-dc2:', 'hdfs://qflock-storage-dc1:')
            print(f'Alter {tables[i].tableName} location to dc1')
            client.alter_table(db_name, tables[i].tableName, tables[i])
        if 'path' in tables[i].sd.serdeInfo.parameters and \
           'hdfs://qflock-storage-dc2:' in tables[i].sd.serdeInfo.parameters['path']:
            tables[i].sd.serdeInfo.parameters['path'] = tables[i].sd.serdeInfo.parameters['path'].replace('hdfs://qflock-storage-dc2:',
                                                                                                          'hdfs://qflock-storage-dc1:')
            client.alter_table(db_name, tables[i].tableName, tables[i])
    for t in tables:
