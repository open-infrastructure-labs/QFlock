...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        dest = sys.argv[1]
    table_config = "store_sales,inventory,store_returns,web_returns,item,customer_address," +\
                   "time_dim,promotion,store,call_center,warehouse,reason"
    table_filters = table_config.split(",")
        ...
        if len(table_filters):
            ...
            for filter in table_filters:
                if filter in tables[i].tableName:
                    ...
                    break
            if found is False:
                # Change all dest -> src tables.
                if f'hdfs://qflock-storage-{dest}:' in tables[i].sd.location:
                    tables[i].sd.location = tables[i].sd.location.replace(
                        f'hdfs://qflock-storage-{dest}:', f'hdfs://qflock-storage-{src}:')
                    print(f'Alter {tables[i].tableName} location to {src}')
                    client.alter_table(db_name, tables[i].tableName, tables[i])
                if 'path' in tables[i].sd.serdeInfo.parameters and \
                        f'hdfs://qflock-storage-{dest}:' in tables[i].sd.serdeInfo.parameters['path']:
                    tables[i].sd.serdeInfo.parameters['path'] = \
                        tables[i].sd.serdeInfo.parameters['path'].replace(f'hdfs://qflock-storage-{dest}:',
                                                                          f'hdfs://qflock-storage-{src}:')
                    client.alter_table(db_name, tables[i].tableName, tables[i])
                continue
        # Change all src -> dest tables.
